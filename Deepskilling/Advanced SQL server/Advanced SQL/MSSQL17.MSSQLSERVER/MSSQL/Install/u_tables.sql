...
(
    ...
    (duration > 15000 and 
        (   
            ...
            or 
            (duration > 30000 /* Waits for locks that have exceeded 30 seconds */
...
(
    action (package0.callstack_rva, sqlserver.session_id, sqlserver.sql_text)
    where 
    (duration > 5000 /* Login related preemptive waits that have exceeded 5 seconds */
        and
        (   
            (   
            ...
            or 
            (duration > 45000 /* Preemptive waits that have exceeded 45 seconds */
                and 
                (   
                    ...
                    or wait_type = N'PREEMPTIVE_OS_GETFINALFILEPATHBYHANDLE'
                )
            )
        )
    )
...
add event sqlserver.security_error_ring_buffer_recorded
    (set collect_call_stack = (0)
        ...
        where not (calling_api_name = N'NLShimImpersonate' and error_code = 5023)),
...
(action (sqlserver.client_app_name, sqlserver.client_hostname, sqlserver.client_pid, sqlserver.query_hash, sqlserver.session_id, sqlserver.session_nt_username, package0.callstack_rva)),
...
/*
Increase file size and retention on Standard and Enterprise editions and in SQL Managed Instance
*/
if serverproperty('EngineEdition') in (2,3,8,10)
begin
...
/*
Extended Events AlwaysOn_health session.
The session start state is based on the previous session start state.
*/
...
/* help debug replica connection timeout issues */
add event ucs.ucs_connection_setup,
/* include HADR tracing messages if trace flag 3605 is enabled */
add event sqlserver.hadr_trace_message,
/* capture sp_server_diagnostics event when STATE is 3 (Error) */
add event sqlserver.sp_server_diagnostics_component_result
    (set collect_data = 1
        where [state] = 3),
add event sqlserver.error_reported
(
    ...
    (
        ...
        /* availability group connection timeout information */
        ...
        /* availability group listener state */
        ...
        /* database replica suspended or resumed */
        or [error_number]=(35264) 
        or [error_number]=(35265)

        /* alter availability group rollback */
        or [error_number]=(41188) 
        or [error_number]=(41189) 

        /* not enough worker threads in the availability group thread pool */
...
/*
Set new AlwaysOn_health session start state based on the previous session state
*/
...
Maintained for historical reasons. The telemetry_xevents session is defined elsewhere.
*/
